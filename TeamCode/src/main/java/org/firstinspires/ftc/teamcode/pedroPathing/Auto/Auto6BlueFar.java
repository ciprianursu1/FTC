...
public class Auto6BlueFar extends OpMode {
    ...
    // BLUE target (field coords)
    private double targetX = 10.0;
    private double targetY = 144.0;

    // TeleOp aiming geometry (with offset)
    ...
    public void init() {
        ...
        pinpoint.setStartPose(new Pose(57, 9, Math.toRadians(90)));
    ...
    public void loop() {
        ...
        // aim -> compute rpm/angle -> flywheel
        updateTurretAim();
        if (aimingEnabled) {
            computeParameters();
        ...
        switch (stage) {
            ...
            case 3: // MOVE UP TO Y=21 (so Path5 starts correctly)
                aimingEnabled = false;
                if (outtakeMode) break;
            ...
            case 4: // GO TO STACK (single pass)
                aimingEnabled = false;
                if (outtakeMode) break;
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            Path9 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(57.000, 9.000), new Pose(57.000, 12.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(293), Math.toRadians(293))
                    .build();

            // move from shoot-line Y=12 up to the “lane” Y=21
            Path3 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(57.000, 12.000), new Pose(57.000, 21.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(90), Math.toRadians(299))
                    .build();

            // single pass (6-ball): use the tighter left-side intake set
            Path5 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(57.000, 21.000), new Pose(12.000, 21.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(299), Math.toRadians(200))
                    .build();

            Path6 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(12.000, 21.000), new Pose(12.000, 12.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(200), Math.toRadians(200))
                    .build();

            Path7 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(12.000, 12.000), new Pose(57.000, 21.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(200), Math.toRadians(299))
                    .build();

            // park (6-ball)
            Path8 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(57.000, 21.000), new Pose(46.000, 21.000)))
                    .setLinearHeadingInterpolation(Math.toRadians(299), Math.toRadians(299))
                    .build();
        }
    }
}
